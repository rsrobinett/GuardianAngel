    ...
    describe('Location Simulator Object', function () {
        ...
        describe('submit a new location', function () {
            it('should submit a location to the db and return a success message', function (done) {
                var post = new device.LocationPush(1);  // make sure deviceID is in database already
                app.con.query(app.queryBuilder.add.deviceData(post.randomize(), app.con), function (err, res) {
                    if (err) {
                        assert(false);
                        done();
                    }
